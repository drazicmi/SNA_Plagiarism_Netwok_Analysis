...
filePath = 'data_set/Results 2_anonymized.csv'
...
# First add each student as a node (39 unique students)
...
# print(graph) -> Graph with 39 nodes and 500 edges
...
plt.savefig(os.path.join("pictures_from_code", "DZ2NetDistribution of Local Clustering Coefficients.png"), bbox_inches='tight')
plt.close()
...
plt.savefig(os.path.join("pictures_from_code", "DZ2NetAssortativity_Degree_Analysis.png"), bbox_inches='tight')
plt.close()
...
plt.ylabel("P(k)")
plt.legend()
plt.savefig(os.path.join("pictures_from_code", "DZ2NetDegree_Distribution_PowerLaw.png"), bbox_inches='tight')
plt.close()
...
nx.write_gexf(graph, os.path.join("gephi_export", "DZ2Net_gephi.gexf"))